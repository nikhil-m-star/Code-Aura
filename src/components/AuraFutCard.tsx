...
export const AuraFutCard: React.FC<AuraFutCardProps> = ({ github, leetcode, ai, cardRef }) => {
  ...
  const realStats = [
    { label: 'STARS', val: formatFullNumber(github.totalStars), icon: Star, color: 'text-amber-400' },
    { label: 'REPOS', val: formatFullNumber(github.publicRepos), icon: FolderGit2, color: 'text-cyan-400' },
    { label: 'SOLVED', val: formatFullNumber(leetcode ? leetcode.totalSolved : 'N/A'), icon: Cpu, color: 'text-emerald-400' },
    { label: 'COMMITS', val: formatFullNumber(github.recentCommitCount), icon: Flame, color: 'text-rose-400' },
      ...
      >
        ...
        <div className="w-full h-full rounded-[28px] bg-[#0c0c0e] p-6 flex flex-col justify-between relative overflow-hidden text-white">
          ...
          <div className="flex flex-col items-center text-center my-3 relative z-10">
            ...
            <h3 className="text-xl sm:text-2xl font-black text-white tracking-tight mt-3 text-center leading-tight">
              {displayName}
            </h3>

            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs font-bold text-gray-400">@{github.username}</span>
              <span className={`text-[10px] font-black px-2.5 py-0.5 rounded-md ${theme.badgeBg} ${theme.badgeText}`}>
                {github.topLanguage}
              </span>
            </div>
          </div>

          {/* ── AI Archetype Banner (Sleek Clean Pill Box, No Borders) ── */}
          <div className="p-3.5 rounded-2xl bg-[#15151a] text-center space-y-0.5 relative z-10">
